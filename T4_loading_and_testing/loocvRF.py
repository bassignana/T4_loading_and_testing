# -*- coding: utf-8 -*-
"""
Created on Tue Aug 31 11:58:06 2021
...
#TODO remove the pid that is used in the random forest model in LOOCV_featureselection function - done in the main function
...
def LOOCV_featureselection(data, ids, outcomevar, dropcols, idcolumn, numestimators=1000):
    """
        ...
        ids = the participant ID of the fold you are on (‘ids’) - its a single string value, not list
            the pid in ids is the one left out
    """
...
def RFLOOCV(data, ids, outcomevar, dropcols, idcolumn, numestimators=1000, fs=0):
    """
        Intermediate function. Please use loocvRF function.
    ...
    filteredi = filteredi['value'] #This is a list of values that are unimportant to drop from your model
...
def loocvRF2(data, idcolumn, outcomevar, dropcols=[], numestimators=1000, fs=0.02):
    """
    ...
    # Forse posso togliere il list interno 
    IDlist = list(set(list(data[idcolumn])))
    drop = [idcolumn]
    drop = drop + dropcols
